...
float _pS1; //for Passing the Value 
...
float _pS2; //for Passing the Value 
...
void setup(void) {
  Serial1.begin(9600); //setup Serial port for sending LED Values
...
void loop() {
  //---------- Switch 0 Functions ----------------// 
  switch0.update(); //update switch state
  pos0_state = switch0.read(); //read switch state
  if(pos0_state != pos0_last_state) { //only run if state is changed
    if(pos0_state == HIGH) { //only run on press, not release
      switch0_timer = millis(); //start hold timer
    }
  }
  if(pos0_state == HIGH && millis() - switch0_timer > 1000) { //if switch has been down for a second
    if(!presetWritten) { //and preset hasn't been written 
      writePreset(presetNumber); //write preset
      presetWritten = 1;
    }
  }
  if(pos0_state == LOW && pos0_last_state == HIGH) { //if switch has been released
    if(!presetWritten) { //and preset has not been written
      recallPreset(presetNumber); //recall preset
    }
    presetWritten = 0;
  }
  ...
  //---------- Switch 1 Functions ----------------// 
  //when switch is pressed recall stored color value and write value to encoder
  switch1.update();
  ...
  if(pos3_state != pos3_last_state) {
    if(pos3_state == HIGH){
      switch3_timer = millis(); //start hold timer
      pos3_counter++;
      pos3_counter %= 2;
  ...
  mod1_dc.amplitude((osc1_amp * (mod1_amplitude)) * mod1_dc_val, 1); //tweak this
  ...
  mod2_dc.amplitude((osc2_amp * (mod2_amplitude)) * mod2_dc_val, 1); //tweak this
  car2_dc.amplitude(osc2_amp, 1);

}

//---------- ---------- Utility Functions ---------------- ----------------// 
//---------- Map Linear to Exponential ----------------// 
